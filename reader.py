...
class ProductReader:
    ...
    def get_product(self, code: str) -> Item:
        ...
        item = self.openff.product.get(code)
        name = item['product']['product_name_en']
        # In case the product name was not filled
        if name == "":
            name = item['product']['ecoscore_data']['agribalyse']['name_en']

        image = item['product']['selected_images']['front']['display']['en']
        eco_grade = item['product']['ecoscore_grade']
        materials = self._get_materials(item)
        categories = self._get_categories(item['product']['categories_tags'])
        new_item = Item(name, image, eco_grade, materials, categories)
        return new_item
